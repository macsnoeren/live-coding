...
namespace SimpleWeb {
  ...
  public:
    ...
    private:
      ...
      void* user_space; // Added by Maurice Snoeren

    public:
      std::function<void(std::shared_ptr<Connection>, void* user_space)> on_open;
      std::function<void(std::shared_ptr<Connection>, std::shared_ptr<Message>, void* user_space)> on_message;
      std::function<void(std::shared_ptr<Connection>, int, const std::string &, void* user_space)> on_close;
      std::function<void(std::shared_ptr<Connection>, const error_code &, void* user_space)> on_error;
      ...
      void *getUserSpace() { return this->user_space; }
      void  setUserSpace( void *user_space ) { this->user_space = user_space; }
    };
  ...
  protected:
        ...
        if(!ec) {
          ...
          // If fragmented message and not final fragment
            ...
            if(endpoint.on_message)
              endpoint.on_message(connection, message, endpoint.getUserSpace());
    ...
    void connection_open(const std::shared_ptr<Connection> &connection, Endpoint &endpoint) const {
      ...
      if(endpoint.on_open)
        endpoint.on_open(connection, endpoint.getUserSpace());
    }
    ...
    void connection_close(const std::shared_ptr<Connection> &connection, Endpoint &endpoint, int status, const std::string &reason) const {
      ...
      if(endpoint.on_close)
        endpoint.on_close(connection, status, reason, endpoint.getUserSpace());
    }
    ...
    void connection_error(const std::shared_ptr<Connection> &connection, Endpoint &endpoint, const error_code &ec) const {
      ...
      if(endpoint.on_error)
        endpoint.on_error(connection, ec, endpoint.getUserSpace());
    }
  };
